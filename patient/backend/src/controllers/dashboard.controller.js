...
const uploadReport = async (req, res, next) => {
  try {
    ...
    // Ensure patient portal account exists: patient can login with registration_number + name (as password)
    const regNo = String(registration_number).trim();
    let patient = await Patient.findOne({ where: { registration_number: regNo } });
    if (!patient) {
      const nameHash = await bcrypt.hash(String(patient_name).trim(), 12);
      patient = await Patient.create({
        name: String(patient_name).trim(),
        registration_number: regNo,
        password_hash: nameHash,
        email: null,
      });
    }

    return res.status(201).json({ success: true, message: 'Report uploaded successfully. Patient can view it in the portal using Register Number and Name.', data: report });
  } catch (err) {
    next(err);
  }
};
